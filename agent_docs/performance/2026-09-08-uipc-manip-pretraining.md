...
| Batched throughput, mixed tshirt_26 / tshirt_392 / hospital_gown slots | 8 slots 480 ms per step, 16 slots 929 ms per step. Read at the time as near-linear saturation; a later profile of the same environment measures strongly sublinear scaling up to 32 copies, so that reading is withdrawn. See the [September 9 record](2026-09-09-dressing-correctness.md) |
...
limit and replayed offline: 0 of 16, ratios 0.0, return -95.6, no early
turn, cuff within 4.1 cm. Q mean ended at 111. Four evaluations over 100k
transitions show no insertion, which is the same outcome the Newton
reference records at ten times that budget.
