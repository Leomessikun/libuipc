...
| [Physics gradients as control signals](2026-09-13-physics-gradients.md) | Level 1 probe run: 3 cells, 7 states, ~45 GPU min | Finite-difference sensitivity of coverage, contact energy and arm force to the gripper command at the expert's elbow, passed and stall states: after the elbow the coverage gradient is a repeatable local derivative and walking it beats the expert and random; at the elbow the coverage reward is flat in every cell; one stall is an environment lock (no-move collision rule, clearance 12.6 mm vs 12 mm) that two composed one-step gradients (release, then advance) escape with +17.5 mm / +0.053 coverage; noise is state-dependent (0.02–23 mm). Level 2 (adjoint in the backend) and Level 3 (SHAC-style actor, h ≥ 4 decisions) designed, not built |
