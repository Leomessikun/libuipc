...
| Record | Status | Scope |
|---|---|---|
| [IAQL direct-picker benchmark](2026-09-14-iaql-benchmark.md) | Gate, fixed-teacher fit and refit control measured; matched 5k-step pair running | Paired SAC value/action-gradient targets, complete-decision x/v tangent, matched SiLU networks: gate 4/4 at friction 0 (the error grows with drag and is the SPD projection, not Newton tolerance), held-out slope cosine 0.38 → 0.84 with paired labels and none with shuffled, friction 0.6 fails the gate 1/4 for want of the lagged friction terms |
