...
def smplx_faces(model_dir: str | None = None, gender: str = "neutral", num_betas: int = 10) -> np.ndarray:
    """The model's triangle topology, which every body it generates shares.

    ``Body`` carries its own copy; this is for the callers that hold only vertices, such as a
    cached cell's ``human_points``.
    """
