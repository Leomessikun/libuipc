...
| `--collision-geometry` | `arm`, `full_body` | `arm` | dressing: use the right-arm mesh or complete fixed SMPL-X mesh for IPC cloth contact. The observation and progress metric remain right-arm based. A checkpoint trained with one collision choice cannot silently resume training with the other. |
