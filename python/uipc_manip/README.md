...
| `--cell-source` | `live`, `cache` | `live` | dressing: `live` drapes each garment online in libuipc and places it on a generated SMPL-X body along the forearm, at a per-cell clearance checked against the whole arm mesh, so any (garment, body) pair is a cell; `cache` reads the Newton bake's pre-worn states |
